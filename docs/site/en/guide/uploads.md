...
Install the matching `@soeditor/file-manager@1.4.0` and register `uploadServiceToken` per instance. `create(request)` returns a task with result, cancel and subscribe.
...
Use `/cms/optional` for external asset plugins in 1.4.0; see the [entry limitation](/en/api/imports#external-asset-plugins).
