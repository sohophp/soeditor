---
title: '上传图片'
description: '安装相同版本的 @soeditor/file-manager@1.2.1，在每个实例上注册 uploadServiceToken。create(request) 返回包含 result、cancel 和 subscribe 的上传任务。'
---

# 上传图片

安装相同版本的 `@soeditor/file-manager@1.2.1`，在每个实例上注册 `uploadServiceToken`。`create(request)` 返回包含 result、cancel 和 subscribe 的上传任务。
...
1.2.1 的外部资源插件使用 `/cms/optional`，原因见[入口限制说明](/zh-CN/api/imports)。
