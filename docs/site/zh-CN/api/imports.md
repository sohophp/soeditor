...
| `@soeditor/react/cms`    | 当前工作区组件，尚未发布；使用 `value/onChange`。 |
| `@soeditor/vue/cms`      | 当前工作区组件，尚未发布；支持 `v-model`。        |

[React 指南](/zh-CN/guide/react) · [Vue 指南](/zh-CN/guide/vue) · [视频指南](/zh-CN/guide/video)。本地框架预览显式使用工作区包，不混入正式发布示例。
